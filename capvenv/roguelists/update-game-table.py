...
def scrap():
  ...
  df.to_csv(f'capvenv/roguelists/{game}.csv', index=False, header=False)
  print('all data was created')


def updateGames():
  # connect to the database
  conn = sqlite3.connect('capvenv/roguelists/db.sqlite3')
  print(conn)
  c = conn.cursor()
  ...
  c.execute('''CREATE TABLE IF NOT EXISTS rogueapp_game
              ...
              genres TEXT)''')
  # read the CSV file
  with open('capvenv/roguelists/roguelike.csv', 'r', encoding='utf-8') as f:
    reader = csv.reader(f)
